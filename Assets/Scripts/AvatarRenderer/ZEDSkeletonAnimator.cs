...
namespace Assets.Scripts.AvatarRenderer
{
    ...
    public class ZEDSkeletonAnimator : MonoBehaviour
    {
        ...
        private void Awake()
        {
            bodyTrackingManager = FindObjectsByType<CustomZedController>().First();
        ...
        {
            foreach (var bone in Skhandler.RigBoneTarget) animator.SetBoneLocalRotation(bone.Key, bone.Value);
        }
                ...
                if (animator)
                {
                    ...
                    if (enableFootIK)
                    {
                        ...
                        if (RightFootTransform != null)
                        {
                            ...
                            // move foot target
                            effectorTargetPos = AccumulateLerpVec3(startLerpPosR, targetLerpPosR, ref curTValR,
                                ref curLerpTimeR);

                            var ray = new Ray(
                                effectorTargetPos + Vector3.up *
                                (groundedR ? thresholdLeaveGroundedState : thresholdEnterGroundedState), Vector3.down);
                            if (Physics.Raycast(ray, out var hit,
                                    2 * (groundedR ? thresholdLeaveGroundedState : thresholdEnterGroundedState),
                                    raycastDetectionLayers))
                            {
                                effectorTargetPos = CustomInterp(startLerpPosR,
                                    hit.point + new Vector3(0, ankleHeightOffset, 0), ref curLerpTimeR);
                                normalR = hit.normal;
                                groundedR = true;
                                animator.SetIKRotation(
                                    AvatarIKGoal.RightFoot,
                                    Quaternion.FromToRotation(
                                        animator.GetBoneTransform(HumanBodyBones.RightFoot).up, normalL) *
                        ...
                        if (LeftFootTransform != null)
                        {
                            ...
                            // move foot target
                            effectorTargetPos = AccumulateLerpVec3(startLerpPosL, targetLerpPosL, ref curTValL,
                                ref curLerpTimeL);

                            posStartRay = effectorTargetPos + Vector3.up *
                                (groundedL ? thresholdLeaveGroundedState : thresholdEnterGroundedState);
                            posHitRay = posStartRay + Vector3.down * 2 *
                                (groundedL ? thresholdLeaveGroundedState : thresholdEnterGroundedState);

                            var ray = new Ray(
                                effectorTargetPos + Vector3.up *
                                (groundedL ? thresholdLeaveGroundedState : thresholdEnterGroundedState), Vector3.down);
                            if (Physics.Raycast(ray, out var hit,
                                    2 * (groundedL ? thresholdLeaveGroundedState : thresholdEnterGroundedState),
                                    raycastDetectionLayers))
                            {
                                posHitRay = hit.point;
                                colorAnkleRAfterMove = Color.white;
                                effectorTargetPos = CustomInterp(startLerpPosL,
                                    hit.point + new Vector3(0, ankleHeightOffset, 0), ref curLerpTimeL);
                                normalL = hit.normal;
                                groundedL = true;
                                animator.SetIKRotation(
                                    AvatarIKGoal.LeftFoot,
                                    Quaternion.FromToRotation(
                                        animator.GetBoneTransform(HumanBodyBones.LeftFoot).up, normalR) *
        ...
        {
            var confAnkleLeft = Skhandler.currentConfidences[Skhandler.currentLeftAnkleIndex];
            var confAnkleRight = Skhandler.currentConfidences[Skhandler.currentRightAnkleIndex];
            ...
            {
                var vL = targetLerpPosL;
                var vR = targetLerpPosR;
        ...
        private Vector3 AccumulateLerpVec3(Vector3 startPos, Vector3 targetPos, ref float curTVal,
            ref float currentLerpTime)
        {
        ...
        private void LateUpdate()
        {
            ...
            if (!enableFootIK) ManageHeightOffset();
        }
        ...
        [Tooltip(
            "EXPERIMENTAL: Filter feet movements caused by root offset when the feet should not be moving (on floor).")]
        public bool filterSlidingMovementsOnGround;
        ...
        [Tooltip(
            "Distance (between ankle and environment under it) under which a foot is considered on the floor. Used to check if the foot is still on the floor.")]
        ...
        [Header("RIG SETTINGS")] public Transform LeftFootTransform = null;

        public Transform RightFootTransform = null;
        public float ankleHeightOffset = 0.102f;
        public CustomZedController bodyTrackingManager;

        [Header("Keyboard controls")] public KeyCode toggleFootIK = KeyCode.I;
        ...
        private readonly float bodyTrackingFrequency = 30f;
        ...
        private readonly float smoothingFactor = 3f;

        #endregion

        #region vars

        public CustomSkeletonHandler Skhandler { get; set; } = null;
        ...
        [Header("Debug")] public float targetLerpPosMultiplier = 1f;
