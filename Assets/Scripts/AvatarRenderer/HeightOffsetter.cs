...
namespace Assets.Scripts.AvatarRenderer
{
    ...
    {
        public float CurrentheightOffset { get; set; }
        ...
        ///     Uses the HeightOffsetStabilized bool value from the ZEDSkeletonAnimator to know when to stop and settle on the
        ///     current offset.
        /// </summary>
        ...
        public Vector3 ComputeRootHeightOffsetXFrames(float confFootL, float confFootR, Vector3 lastPosFootL,
            Vector3 lastPosFootR, float ankleHeightOffset)
        {
            var offsetToApply = new Vector3(0, CurrentheightOffset, 0);
            ...
            if (automaticOffset)
            {
                ...
                {
                    var rayL = new Ray(lastPosFootL + Vector3.up * findFloorDistance, Vector3.down);
                    var rayUnderFootHitL = Physics.Raycast(rayL, out var hitL, findFloorDistance * 2, layersToHit);
                    var rayR = new Ray(lastPosFootR + Vector3.up * findFloorDistance, Vector3.down);
                    var rayUnderFootHitR = Physics.Raycast(rayR, out var hitR, findFloorDistance * 2, layersToHit);
                    ...
                    {
                        minFootFloorDistance =
                            -1.0f * Mathf.Max(Mathf.Abs(footFloorDistanceL), Mathf.Abs(footFloorDistanceR));
        ...
        public void Update()
        {
            ...
            else if (Input.GetKeyDown(decreaseOffsetKey)) manualOffset.y -= offsetStep;

            if (Input.GetKeyDown(toggleAutomaticOffsetKey)) automaticOffset = !automaticOffset;
        }

        #region vars

        private ZEDSkeletonAnimator zedSkeletonAnimator;

        [Header("Main settings")] [Tooltip("Height offset applied to transform each frame.")]
        ...
        private LinkedList<float> feetOffsetBuffer = new();
        ...
        [Header("Keyboard controls")] public KeyCode increaseOffsetKey = KeyCode.UpArrow;
