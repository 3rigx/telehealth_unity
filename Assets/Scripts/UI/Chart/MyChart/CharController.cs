...
namespace Assets.Scripts.UI.Chart.MyChart
{
    public class SkeletonChartController : MonoBehaviour
    {
        ...
        private Dictionary<int, LineRenderer[]> joints = new ();
        ...
        private LineRenderer[] make3LineRenderers()
        {
            var x = Instantiate(linePrefab).GetComponent<LineRenderer>();
            var y = Instantiate(linePrefab).GetComponent<LineRenderer>();
            var z = Instantiate(linePrefab).GetComponent<LineRenderer>();
            x.color = Color.red;
            y.color = Color.green;
            z.color = Color.blue;
        ...
        public void setBodyFormat(BodyFormat value)
        {
            ...
            {
                LineRenderer[] positionGroup = make3LineRenderers();
                selector.addGroup(k + "Position", positionGroup);
                
              
                LineRenderer[] rotationGroup =  make3LineRenderers();
                selector.addGroup(k + "Rotation", rotationGroup);

                LineRenderer[] avgPosGroup = make3LineRenderers();
                selector.addGroup(k + "AvgPos", avgPosGroup);

                LineRenderer[] avgSpeedGroup = make3LineRenderers();
                selector.addGroup(k + "AvgVel", avgSpeedGroup);

                LineRenderer[] avgAccGroup = make3LineRenderers();
                selector.addGroup(k + "AvgAcc", avgAccGroup);

                LineRenderer[] avgJerkGroup = make3LineRenderers();
                selector.addGroup(k + "AvgJerk", avgJerkGroup);

                LineRenderer[] avgEulerGroup = make3LineRenderers();
                selector.addGroup(k + "AvgEuler", avgEulerGroup);

                LineRenderer[] avgRotGroup = make3LineRenderers();
                selector.addGroup(k + "AvgRot", avgRotGroup);

                LineRenderer[] avgAngularSpeedGroup = make3LineRenderers();
                selector.addGroup(k + "AvgAngularSpeed", avgAngularSpeedGroup);

                LineRenderer[] avgAngularAccGroup = make3LineRenderers();
                selector.addGroup(k + "AvgAngularAcc", avgAngularAccGroup);

                LineRenderer[] avgAngularJerkGroup = make3LineRenderers();
                selector.addGroup(k + "AvgAngularJerk", avgAngularJerkGroup);

                joints.Add(v,positionGroup.Concat(rotationGroup).Concat(avgPosGroup).Concat(avgSpeedGroup).Concat(avgAccGroup).Concat(avgJerkGroup).Concat(avgEulerGroup).Concat(avgRotGroup).Concat(avgAngularSpeedGroup).Concat(avgAngularAccGroup).Concat(avgAngularJerkGroup).ToArray());
            }
        }
        ...
        public int cursor
        {
            ...
            {
                _cursor = value;
                foreach (LineRenderer[] joint in joints.Values)
                {
                    foreach (LineRenderer line in joint)
                    {
                        line.cursor = value;
                    }
                }
